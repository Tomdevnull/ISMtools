...
[.lead]
*Automation tools for https://eu.fsastech.com/eu/products-services/primergy-servers/infrastructure-manager-ism/ [Infrastructure Manager (ISM)] using REST API.*
...
+ Additional option for `ism_get_sysrep` to upload it to 
Fsas Technologies SFTP server +
...
Creates and downloads a System-Report ZIP file which contains the system report and the system event log (SEL). If no outputfile is given then default value _OUTPUTFILE_ defined in <<Configfile>> is used. With option `-c` the output file is copied afterwards to Fsas Technologies SFTP server to directory `/incoming`.
...
Copies one or more files to the `/incoming` directory of Fsas Technologies SFTP server. If asked you can use _ftp_ as password.
...
== Bibliography
[bibliography]
- [[[ISM_REST_API]]] Fsas Technologies: https://support.ts.fujitsu.com/dl?ID=1bc17707-0d8a-4dda-81b3-a06bd7e0910b[REST API Reference Manual]
